...
- [Neon & glow](#neon--glow): [Neon border utility](#neon-border-utility) · [Neon card](#neon-card) · [Glow-on-hover card](#glow-on-hover-card) · [Gradient-border card](#gradient-border-card) · [BorderGlow card (React Bits)](#borderglow-card-react-bits) · [Neon button](#neon-button)
...
### BorderGlow card (React Bits)

Interactive glow effect that follows the cursor near card edges. Uses mesh gradient border with directional cone mask.

**Install:** Copy `BorderGlow.tsx` and `BorderGlow.css` into `components/ui/border-glow/`.

```tsx
// components/ui/border-glow/BorderGlow.tsx
...
**CSS:** Copy `BorderGlow.css` into the same directory.
...
>
  <div className="p-6">
    ...
    <p className="mt-2 text-sm text-ink-3">Card content here.</p>
  </div>
</BorderGlow>
```
...
| glowColor | string | "40 80 80" | HSL values for the glow color, as "H S L". |
...
| colors | string[] | [...] | Array of 3 hex colors for the mesh gradient border. |
